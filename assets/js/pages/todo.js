(function () {
  ...
  const backupModalFile = document.getElementById('backup-modal-file');
  ...
  function doBackupExport() {
    if (!window.Storage) { return; }
    if (window.Storage.mode === 'demo') { flashSync('Log in to export', true); return; }
    flashSync('Preparing backup\u2026', false);
    window.Storage.exportAll().then(function (data) {
      ...
      flashSync('Backup downloaded \u2713', false);
    })['catch'](function (err) {
  ...
  function selectedBackupMode() {
    const checked = document.querySelector('input[name="backup-mode"]:checked');
  ...
  function handleBackupFile(file) {
    ...
    reader.onload = function () {
      ...
      try {
        ...
        flashSync('Invalid backup file', true);
        return;
      }
      if (!parsed ||
          Object.prototype.toString.call(parsed.tasks) !== '[object Array]' ||
          !parsed.days || typeof parsed.days !== 'object') {
        flashSync('Not a Habit Board backup', true);
        return;
      }
      openBackupModal(parsed, file.name);
    };
    reader.onerror = function () { flashSync('Could not read file', true); };
    reader.readAsText(file);
  }

  if (backupExportBtn) {
    backupExportBtn.addEventListener('click', doBackupExport);
  }
  if (backupImportBtn) {
    backupImportBtn.addEventListener('click', () => {
      if (window.Storage && window.Storage.mode === 'demo') {
        flashSync('Log in to import', true);
        return;
      }
  ...
  if (backupConfirmBtn) {
    backupConfirmBtn.addEventListener('click', () => {
      ...
      window.Storage.importAll(pendingBackup, mode).then(function (summary) {
        ...
        loadTasks();
        renderTaskList();
        flashSync('Imported ' + summary.tasks + ' tasks \u2713', false);
      })['catch'](function (err) {
        if (backupModalStatus) {
